...
const ComplainTable = ({ headings, complains, show, setshow }) => {
 ...
 return (
  <>
   <Navbar />
   <div className="p-5">
    <Button variant="outlined" color="green" className="w-full h-10 " onClick={handleAddComplain}>
     Add new complaint
    </Button>
   </div>
   <div className="px-5">
         ...
         {complains?.map((item) => (
          ...
          >
           <div className="flex items-baseline gap-5 text-[24px] ">
            ...
            <div className="">
             <div className="flex justify-end text-[16px]">({item?.status?.name})</div>
           ...
           <div className="text-black">{item.description?.substring(0, 40)}</div>

           {show.id === item._id && show.display && (
            <div className="text-black">{item.description?.substring(40)}</div>
           )}
           ...
           {item.description.length > 40 && (
            ...
            >
             {show.id === item._id && show.display ? "See Less" : "...See More"}
            </div>
           )}
          </div>
         ))}
